...
def _define_profiles(
    energy_dict: Dict[str, float], energy_type: str
) -> List[Dict[str, Any]]:
    """
    ...
    if reactant_key not in energy_dict:
        raise KeyError(
            f"Reactant stage not found. Available keys: {list(energy_dict.keys())}"
        )
    if product_key not in energy_dict:
        raise KeyError(
            f"Product stage not found. Available keys: {list(energy_dict.keys())}"
        )
    ...
    for prof in profiles:
        missing_keys = [
            stage["key"] for stage in prof["profile"] if stage["key"] not in energy_dict
        ]
        if not missing_keys:
            valid_profiles.append(prof)
        else:
            logging.info(
                f"Skipping {prof['name']} profile - missing stages: {missing_keys}"
            )

    # If no profiles are valid, log available keys for debugging
    if not valid_profiles:
        logging.info(
            f"No complete profiles can be plotted. Available energy keys: {sorted(energy_dict.keys())}"
        )
...
def _plot_energy_profiles(
    ...
    for prof in profiles:
        _plot_single_profile(
            ax, prof["profile"], energy_dict, prof["color"], prof["annotate_rp"]
        )
    ...
    ax.set_ylabel(
        rf"$\Delta {display_energy_type}$ ({unit})", fontsize=20, fontweight="bold"
    )
                ...
                for catalyst, catalyst_data in catalyst_profiles.items():
                    ...
                    for energy_type in ["E", "G", "G_trans"]:
                        ...
                        try:
                            ...
                            energy_dict = _convert_to_energy_dict(
                                profile_data, profile_key, unit
                            )
                            if not energy_dict:
                                continue
                            ...
                            if energy_type == "G_trans":
                                ...
                                trans_energy_dict = _convert_to_energy_dict(
                                    trans_profile_data, "G_trans", unit
                                )
                                if trans_energy_dict:
                                    normalized_trans_energy = _normalize_energies(
                                        trans_energy_dict
                                    )
                                    ...
                                    if "preTS_full_cat" in normalized_trans_energy:
                                        trans_overlay_values["preTS"] = (
                                            normalized_trans_energy["preTS_full_cat"]
                                        )
                                    if "TS_full_cat" in normalized_trans_energy:
                                        trans_overlay_values["TS"] = (
                                            normalized_trans_energy["TS_full_cat"]
                                        )
                                    if "postTS_full_cat" in normalized_trans_energy:
                                        trans_overlay_values["postTS"] = (
                                            normalized_trans_energy["postTS_full_cat"]
                                        )

                                    product_key = (
                                        "Products"
                                        if "Products" in normalized_trans_energy
                                        else "Product"
                                    )
                                    if product_key in normalized_trans_energy:
                                        trans_overlay_values["product"] = (
                                            normalized_trans_energy[product_key]
                                        )
                            ...
                            logging.error(
                                f"Failed to plot {calc_mode} {energy_type} profile for {catalyst} in {combo_name}: {e}"
                            )
                            continue
