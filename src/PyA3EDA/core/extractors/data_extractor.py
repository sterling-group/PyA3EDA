...
from PyA3EDA.core.parsers.qchem_result_parser import (
    ...
    parse_smd_cds_extended_print,
    parse_smd_detail_block,
...
def extract_opt_thermodynamic_data(
...
def extract_sp_thermodynamic_data(
...
def extract_smd_detail_block_data(content: str) -> Dict[str, Any]:
    ...
    if (
        "g_s_final" in smd_data
        and "g_enp_final" in smd_data
        and "cds_detail_final" in smd_data
    ):
...
def extract_cds_extended_print(sp_content: str) -> Dict[str, Any]:
    ...
    if sp_cds_kcal is None:
        logging.warning(
            "Failed to extract CDS from SP extended print. Use print=2 in smx block."
        )
        return {}
    ...
    data.update({"G_CDS (Ha)": sp_cds_ha, "G_CDS (kcal/mol)": sp_cds_kcal})
...
def validate_cds_against_opt(sp_cds_kcal: float, opt_content: str) -> Dict[str, Any]:
    ...
    if not is_valid:
        logging.warning(
            f"CDS validation failed: SP={sp_cds_kcal:.6f}, OPT={opt_cds_kcal:.6f}, diff={cds_diff:.6f}"
        )
    else:
        logging.debug(
            f"CDS validation passed: SP={sp_cds_kcal:.6f}, OPT={opt_cds_kcal:.6f}, diff={cds_diff:.6f}"
        )
...
def _extract_eda_sp(
    sp_content: str, calc_type: str, metadata: Dict[str, Any], opt_content: str = None
) -> Dict[str, Any]:
    ...
    # Set final SP energy
    data.update({"SP_E (Ha)": final_energy_ha, "SP_E (kcal/mol)": final_energy_kcal})
...
def calculate_enthalpy_and_gibbs(data: Dict[str, Any], mode: str) -> None:
    ...
    if base_energy_key in data and "Total Enthalpy Corr. (kcal/mol)" in data:
        ...
        logging.debug(
            f"{mode.upper()}: Calculated H = {data['H (kcal/mol)']:.6f} kcal/mol"
        )
    else:
        logging.debug(
            f"{mode.upper()}: Skipping H calculation - missing energy or enthalpy correction"
        )
    ...
    if not all(k in data for k in required_keys):
        ...
        logging.debug(
            f"{mode.upper()}: Skipping G calculation - missing keys: {missing_keys}"
        )
        return

    temperature = data["Temperature (K)"]
    g_gas = (
        data["H (kcal/mol)"] - temperature * data["Total Entropy Corr. (kcal/mol.K)"]
    )
    data["G(gas) (kcal/mol)"] = g_gas
    data["G (kcal/mol)"] = g_gas
    logging.debug(
        f"{mode.upper()}: Calculated G(gas) = {g_gas:.6f} kcal/mol at T={temperature} K"
    )
    ...
    if solvent != "gas" and all(
        k in data for k in ["Temperature (K)", "Pressure (atm)"]
    ):
        ...
        logging.debug(
            f"{mode.upper()}: Applied standard state correction for {solvent}: "
            f"dG = {correction:.6f} kcal/mol (T={temperature} K, P={pressure} atm)"
        )
        logging.debug(
            f"{mode.upper()}: G(gas) = {g_gas:.6f} kcal/mol,\n G(1M) = {g_solvent:.6f} kcal/mol"
        )

        # No standard-state correction is applied to G_trans by design.
    elif solvent != "gas":
        logging.debug(
            f"{mode.upper()}: Skipping standard state correction for {solvent} - missing temperature or pressure"
        )
    else:
        logging.debug(
            f"{mode.upper()}: Gas phase calculation - no standard state correction applied"
        )
...
def extract_all_data(
    config_manager, system_dir: Path, criteria: str = "SUCCESSFUL"
        ...
        for file_info in iter_input_paths(
            ...
            if file_info and hasattr(file_info, "metadata"):
                ...
                if method_combo:
                    ...
                    combo_files[method_combo].append(
    ...
    logging.info(
        f"Found {len(combo_files)} method combos: {sorted(combo_files.keys())}"
    )
    ...
    for combo_name, input_files in combo_files.items():
        ...
        sp_files = [
            (path, meta) for path, meta in input_files if meta.get("Mode") == "sp"
        ]
        ...
        for input_path, metadata in sp_files:
            ...
            if not opt_content:
                ...
                logging.debug(
                    f"Available OPT keys in cache: {list(opt_content_cache.keys())}"
                )
            else:
        ...
        combo_data = {"opt_data": opt_data, "sp_data": sp_data, "xyz_data": xyz_data}

        if any(combo_data.values()):
            all_extracted_data[combo_name] = combo_data
            logging.info(
                f"Extracted from {combo_name}: {len(opt_data)} OPT, {len(sp_data)} SP, {len(xyz_data)} XYZ"
            )
    ...
    if all_extracted_data:
        ...
        total_files = sum(
            len(combo_data["opt_data"])
            + len(combo_data["sp_data"])
            + len(combo_data["xyz_data"])
            for combo_data in all_extracted_data.values()
        )
        logging.info(
            f"Extraction completed: {total_combos} method combos, {total_files} total files"
        )
