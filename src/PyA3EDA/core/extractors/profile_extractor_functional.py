...
def _get_energy(
    species: str, energy_lookup: Dict[str, Dict[str, float]], calc_type: str = None
...
def _create_stage(
    ...
    energy_lookup: Dict[str, Dict[str, float]],
    ...
    if len(set(non_empty_calc_types)) > 1:
        logging.warning(
            f"Mixed calc_types in '{stage_name}': {set(non_empty_calc_types)}"
        )
...
def _generate_stages(
    ...
    stage_configurations = {
        "reactants": {
            "branch": "reactants",
            "category": "no_cat",
            "stage_name": "Reactants",
            "needs_missing_components": True,
            "needs_catalyst": True,
            "components_key": "all_reactants",
        },
        "products": {
            "branch": "products",
            "category": "no_cat",
    ...
    for entry in entries:
        ...
        if stage_config.get("needs_missing_components") and stage_config.get(
            "components_key"
        ):
            components_list = components[stage_config["components_key"]]
            present_components = entry.get(
                stage_config["components_key"].replace("all_", ""), []
            )
            missing_components = [
                c for c in components_list if c not in present_components
            ]
        ...
        if stage_config.get("needs_catalyst") and not stage_config.get(
            "catalyst_present"
        ):
...
def _generate_catalyst_profile(
    catalyst: str,
    raw_data: List[Dict[str, Any]],
    components: Dict[str, List[str]],
    energy_lookup: Dict[str, Dict[str, float]],
    ...
    profile.extend(
        _generate_stages("reactants", catalyst, raw_data, components, energy_lookup)
    )
    profile.extend(
        _generate_stages("preTS", catalyst, raw_data, components, energy_lookup)
    )
    profile.extend(
        _generate_stages("ts_cat", catalyst, raw_data, components, energy_lookup)
    )
    profile.extend(
        _generate_stages("ts_nocat", catalyst, raw_data, components, energy_lookup)
    )
    profile.extend(
        _generate_stages("postTS", catalyst, raw_data, components, energy_lookup)
    )
    profile.extend(
        _generate_stages("products", catalyst, raw_data, components, energy_lookup)
    )
    ...
    prets_full_stage = next(
        (
            s
            for s in profile
            if s.get("Stage") == "preTS"
            and s.get("Calc_Type") == "full_cat"
            and s.get("G_trans (kcal/mol)") is not None
        ),
        None,
    )
    if prets_full_stage:
...
def _filter_profile(
    profile: List[Dict[str, Any]], energy_type: str
) -> List[Dict[str, Any]]:
    ...
    if not has_energy_data:
        logging.info(
            f"Skipping {energy_type} profile filtering - no {energy_type} data available (SP-only data)"
        )
        return []
    ...
    for stage_name, stages in stage_groups.items():
        ...
        calc_type_stages = [
            s
            for s in stages
            if s.get("Calc_Type") and s.get("Calc_Type") not in [None, "", "unknown"]
        ]
        no_calc_type_stages = [
            s
            for s in stages
            if not s.get("Calc_Type") or s.get("Calc_Type") in [None, "", "unknown"]
        ]

        # Handle calc_type subgroup: smart filtering via full_cat
        if calc_type_stages:
            full_cat_stages = [
                s for s in calc_type_stages if s.get("Calc_Type") == "full_cat"
            ]
            if full_cat_stages:
                # Find min full_cat, keep same species for pol/frz_cat
                min_full_cat = min(
                    full_cat_stages, key=lambda x: x.get(energy_key, float("inf"))
                )
                ...
                for calc_type in ["pol_cat", "frz_cat", "trans_cat"]:
                    matching_stages = [
                        s
                        for s in calc_type_stages
                        if s.get("Calc_Type") == calc_type
                        and s.get("Species") == min_species
                    ]
                    if matching_stages:
                ...
                min_stage = min(
                    calc_type_stages, key=lambda x: x.get(energy_key, float("inf"))
                )
        ...
        if no_calc_type_stages:
            min_no_calc = min(
                no_calc_type_stages, key=lambda x: x.get(energy_key, float("inf"))
            )
...
def extract_profiles(
    raw_data_list: List[Dict[str, Any]], filter_duplicates: bool = False
    ...
    for catalyst in components["all_catalysts"]:
        raw_profile = _generate_catalyst_profile(
            catalyst, raw_data_list, components, energy_lookup
        )
    ...
    for combo_name, combo_data in raw_data.items():
        ...
        for data_key in ["opt_data", "sp_data"]:
            if combo_data.get(data_key):
                ...
                logging.info(
                    f"Generating profiles for {calc_type} data in {combo_name}"
                )
                profiles = extract_profiles(
                    combo_data[data_key], filter_duplicates=True
                )
                if profiles:
                    logging.info(
                        f"  Generated profiles for {len(profiles)} catalysts: {list(profiles.keys())}"
                    )
                else:
                    logging.warning(
                        f"  No profiles generated for {calc_type} data (check if catalyst metadata exists)"
                    )
