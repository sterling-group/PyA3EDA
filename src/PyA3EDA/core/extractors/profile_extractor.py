...
class ProfileExtractor:
    ...
    def _build_energy_lookup(self) -> Dict[str, Dict[str, float]]:
        ...
        for data in self.raw_data:
            ...
            g_trans_val = data.get("G_trans (kcal/mol)")

            if e_val is not None and g_val is not None:
                calc_type = data.get("Calc_Type", "")

                entry = {"E": e_val, "G": g_val}
                if g_trans_val is not None:
                    entry["G_trans"] = g_trans_val
    ...
    def _get_energy(
        self, species: str, calc_type: str = None
    ...
    def _find_entries(
        self, branch: str = None, category: str = None, catalyst: str = None
    ) -> List[Dict[str, Any]]:
    ...
    def _create_stage(
        self, stage_name: str, species_list: List[str], calc_types: List[str] = None
        ...
        Returns:
            Optional[Dict[str, Any]]: Dictionary containing stage data with keys:
                ...
                - 'G_trans (kcal/mol)': Translational Gibbs contribution (if available)
        ...
        for species, calc_type in zip(species_list, calc_types):
            ...
            total_e += energy["E"]
            total_g += energy["G"]
            # Sum G_trans if available
            if "G_trans" in energy:
        ...
        # Add G_trans if available for this stage
        if has_g_trans:
    ...
    def _generate_stages(self, stage_type: str, catalyst: str) -> List[Dict[str, Any]]:
        ...
        for entry in entries:
            ...
            if stage_config.get("needs_missing_components") and stage_config.get(
                ...
                missing_components = [
                    c for c in components if c not in present_components
                ]
    ...
    def _filter_profile(
        self, profile: List[Dict[str, Any]], energy_type: str
    ) -> List[Dict[str, Any]]:
        ...
        for stage_name, stages in stage_groups.items():
            ...
            calc_type_stages = [
                ...
                for s in stages
                if s.get("Calc_Type")
                and s.get("Calc_Type") not in [None, "", "unknown"]
            ]
            no_calc_type_stages = [
                s
                for s in stages
        ...
        for catalyst in self.components["all_catalysts"]:
            ...
            if raw_profile:  # Only include non-empty profiles
                ...
                if filter_duplicates:
                    ...
                    catalyst_profiles["G_trans"] = self._filter_profile(
                        raw_profile, "G_trans"
                    )
    ...
    def process_all_profiles(
        cls, raw_data: Dict[str, Dict[str, List[Dict[str, Any]]]]
        ...
        for combo_name, combo_data in raw_data.items():
            ...
            for data_key in ["opt_data", "sp_data"]:
                if combo_data.get(data_key):
                    ...
                    processed_data[combo_name]["profiles"][data_key] = (
                        extractor.extract_profiles(filter_duplicates=True)
                    )
