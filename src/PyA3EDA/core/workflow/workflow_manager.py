...
class WorkflowManager:
    ...
    def generate_inputs(self) -> None:
        ...
        sp_strategy = (
            getattr(self.args, "sp_strategy", "smart") if self.args else "smart"
        )

        builder.generate_all_inputs(
            self.config_manager, self.system_dir, overwrite, sp_strategy
        )
    ...
    def run_calculations(self) -> None:
        ...
        run_all_calculations(self.config_manager, self.system_dir, run_criteria)
    ...
    def extract_data(self) -> None:
        ...
        from PyA3EDA.core.extractors.delta_delta_extractor import (
            extract_all_delta_delta,
        )
        from PyA3EDA.core.extractors.profile_extractor_functional import (
        ...
        export_all_data(
            processed_data, self.system_dir, delta_delta_data, catalyst_order
        )

        # Generate profile plots by default (can be disabled with --no-plots)
        generate_plots = (
            not getattr(self.args, "no_plots", False) if self.args else True
        )
        if generate_plots:
        ...
        generate_barplots = (
            not getattr(self.args, "no_barplots", False) if self.args else True
        )
