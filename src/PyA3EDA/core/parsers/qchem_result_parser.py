...
PATTERNS = {
    ...
    "final_energy": re.compile(
        r"Final energy is\s+([-+]?\d+\.\d+)(?:\s+([A-Za-z][A-Za-z0-9./\-]*))?\s*$",
        re.MULTILINE,
    ),
    "total_energy": re.compile(
        r"Total energy =\s+([-+]?\d+\.\d+)(?:\s+([A-Za-z][A-Za-z0-9./\-]*))?\s*$",
        re.MULTILINE,
    ),
    ...
    "total_enthalpy_fallback": re.compile(
        r"Total Enthalpy:\s+([-+]?\d+\.\d+)\s+([A-Za-z][A-Za-z0-9./\-]*)", re.MULTILINE
    ),
    ...
    "total_entropy_fallback": re.compile(
        r"Total Entropy:\s+([-+]?\d+\.\d+)\s+([A-Za-z][A-Za-z0-9./\-]*)", re.MULTILINE
    ),
    ...
    "smd_g_enp": re.compile(
        r"\(3\)\s+G-ENP\(liq\) elect-nuc-pol free energy of system\s+([-+]?\d+\.\d+)\s+(a\.u\.)",
        re.MULTILINE,
    ),
    "smd_g_s": re.compile(
        r"\(6\)\s+G-S\(liq\) free energy of system\s+([-+]?\d+\.\d+)\s+(a\.u\.)",
        re.MULTILINE,
    ),
    "smd_cds_detail": re.compile(
        r"\(4\)\s+G-CDS\(liq\) cavity-dispersion-solvent structure\s+([-+]?\d+\.\d+)\s+(kcal/mol)",
        re.MULTILINE,
    ),
    "smd_cds_summary": re.compile(
        r"G_CDS\s+=\s+([-+]?\d+\.\d+)\s+(kcal/mol)", re.MULTILINE
    ),
    "smd_cds_extended_print": re.compile(
        r"Total:\s+([-+]?\d+\.\d+)\s*\n\s*-+", re.MULTILINE
    ),
    # EDA-specific patterns
    "eda_polarized_energy": re.compile(
        r"Energy prior to optimization \(guess energy\)\s*=\s*([-+]?\d+\.\d+)",
        re.MULTILINE,
    ),
    "eda_convergence_energy": re.compile(
        ...
        if isinstance(last_match, tuple):
            for group_idx, field_name in field_mapping.items():
                if group_idx <= len(last_match):
                    value = last_match[
                        group_idx - 1
                    ]  # findall is 0-indexed, field_mapping is 1-indexed
                    try:
                        if "." in str(value):
            ...
            if 1 in field_mapping:
                try:
                    result[field_mapping[1]] = (
                        float(last_match) if "." in str(last_match) else int(last_match)
                    )
                except (ValueError, TypeError):
...
def parse_final_energy(content: str, prefix: str = "E") -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["final_energy"], default_unit="Ha"
    )
...
def parse_total_energy(content: str, prefix: str = "E") -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["total_energy"], default_unit="Ha"
    )
...
def parse_enthalpy(content: str) -> Optional[Dict[str, Any]]:
    ...
    if result is not None:
        ...
        return {
            "Total Enthalpy Corr. (kcal/mol)": convert_unit(
                enthalpy_value, enthalpy_unit, "kcal/mol"
            ),
...
def parse_entropy(content: str) -> Optional[Dict[str, Any]]:
    ...
    if result is not None:
        ...
        return {
            "Total Entropy Corr. (kcal/mol.K)": convert_unit(
                entropy_value, entropy_unit, "kcal/mol.K"
            ),
...
def parse_translational_entropy(content: str) -> Optional[Dict[str, Any]]:
    ...
    if result is not None:
        entropy_value, entropy_unit = result
        return {
            "S_trans (kcal/mol.K)": convert_unit(
                entropy_value, entropy_unit, "kcal/mol.K"
            )
        }
    return None
...
def parse_optimization_status(content: str) -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["optimization_status"], field_mapping={1: "status"}
    )
...
def parse_thermodynamic_conditions(content: str) -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content,
        PATTERNS["thermodynamics"],
        field_mapping={1: "temperature", 2: "pressure"},
    )
    return (
        {"Temperature (K)": result["temperature"], "Pressure (atm)": result["pressure"]}
        if result
        else None
    )
...
def parse_qrrho_parameters(content: str) -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["qrrho_parameters"], field_mapping={1: "alpha", 2: "omega"}
    )
    return (
        {"Alpha": result["alpha"], "Omega (cm^-1)": result["omega"]} if result else None
    )
...
def parse_imaginary_frequencies(content: str) -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["imaginary_frequencies"], field_mapping={1: "count"}
    )
...
def parse_zero_point_energy(content: str) -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["zero_point_energy"], default_unit="kcal/mol"
    )
...
def parse_smd_detail_block(content: str) -> Optional[Dict[str, Any]]:
    ...
    g_enp_result, _ = extract_with_pattern(
        content, PATTERNS["smd_g_enp"], default_unit="Ha"
    )
    ...
    g_s_result, _ = extract_with_pattern(
        content, PATTERNS["smd_g_s"], default_unit="Ha"
    )
    ...
    cds_detail_result, _ = extract_with_pattern(
        content, PATTERNS["smd_cds_detail"], default_unit="kcal/mol"
    )
...
def parse_eda_polarized_energy(
    content: str, prefix: str = "SP_E"
) -> Optional[Dict[str, Any]]:
    """Parse polarized energy from EDA SP calculations (pol types)."""
    result, _ = extract_with_pattern(
        content, PATTERNS["eda_polarized_energy"], default_unit="Ha"
    )
...
def parse_eda_convergence_energy(
    content: str, prefix: str = "SP_E"
) -> Optional[Dict[str, Any]]:
    """Parse full energy from EDA SP calculations (frz/full type)."""
    result, _ = extract_with_pattern(
        content, PATTERNS["eda_convergence_energy"], default_unit="Ha"
    )
...
def parse_bsse_energy(content: str) -> Optional[Dict[str, Any]]:
    ...
    result, _ = extract_with_pattern(
        content, PATTERNS["bsse_energy"], default_unit="kJ/mol"
    )
